    ...
    for (auto iterator = inputAccumulator->EventsBegin (); iterator != inputAccumulator->EventsEnd (); ++iterator)
    {
        ...
        case InputStorage::InputEventType::KEYBOARD:
        {
            if (auto mapIterator = keyMap.find (event.keyboard.keyCode); mapIterator != keyMap.end ())
            {
                ...
                io.AddKeyEvent (imGuiKey, event.keyboard.keyState == InputStorage::KeyState::DOWN);
        ...
        case InputStorage::InputEventType::MOUSE_BUTTON:
            ...
            if (event.mouseButton.state == InputStorage::KeyState::UP ||
                isInViewport (event.mouseButton.mouseX, event.mouseButton.mouseY))
            {
                ...
                {
                case InputStorage::MouseButton::LEFT:
                    io.AddMouseButtonEvent (ImGuiMouseButton_Left,
                                            event.mouseButton.state == InputStorage::KeyState::DOWN);
                    break;

                case InputStorage::MouseButton::MIDDLE:
                    io.AddMouseButtonEvent (ImGuiMouseButton_Middle,
                                            event.mouseButton.state == InputStorage::KeyState::DOWN);
                    break;

                case InputStorage::MouseButton::RIGHT:
                    io.AddMouseButtonEvent (ImGuiMouseButton_Right,
                                            event.mouseButton.state == InputStorage::KeyState::DOWN);
                    break;

                case InputStorage::MouseButton::X1:
                case InputStorage::MouseButton::X2:
